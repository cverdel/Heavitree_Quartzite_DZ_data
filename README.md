...
tempdir()
```
This results in two csv files in the temp directory (Heavitree_sample_summary_w_geographic_MDS.csv and Synthetic_spectra_w_geographic_MDS.csv) that can be plotted with GIS. Below is a map of these data made using QGIS. Triangles are synthetic spectra, and circles are samples (note that the locations of samples on this map aren't their actual geographic locations, although they are shown in the correct mapsheet). Plotting the data in this way illustrates the variation in detrital zircon populations highlighted by MDS, as well as geospatial variation.
